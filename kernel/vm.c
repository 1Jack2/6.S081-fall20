...
/*
 * create a direct-map kernel page table for proc's.
 */
pagetable_t
proc_kpagetable()
{
...
// Recursively free page-table pages.
// However do not free leaf physical memory pages.
void
proc_kfreewalk(pagetable_t pagetable)
{
  // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
...
void 
vmprint0(pagetable_t pagetable, int depth)
{
  // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
      ...
      uint64 child = PTE2PA(pte);
      vmprint0((pagetable_t)child, depth + 1);
    } else if(pte & PTE_V){
      for (int i = 0; i < depth; i++) {
        printf(".. ");
      }
